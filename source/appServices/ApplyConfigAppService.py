...
class ApplyConfigAppService ():
	...
	def run (self):
		...
		print('{0}: Applying config {1}.'.format(databaseSymbolName if targetSymbolType == SymbolType.Database else serverSymbolName, configSymbolName))
		...
		scriptRunnerService.runConfigApplyScripts()

		print('{0}: Apply complete.'.format(databaseSymbolName if targetSymbolType == SymbolType.Database else serverSymbolName))
